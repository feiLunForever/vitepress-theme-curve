...
/**
 * 获取时间已过的函数
 * @return {Object} 包含day、week、month和year的已过时间信息
 */
export const getTimePassed = () => {
  ...
  const getDifference = (unit) => {
    ...
    const passed = now.diff(start, unit === "day" ? "hour" : "day");
...
export const getDaysSince = (dateStr) => {
  ...
  targetDate.add(years, 'year'); // 更新目标日期到当前年份
  const months = now.diff(targetDate, 'month');
  targetDate.add(months, 'month'); // 更新目标日期到当前月份
